...
# === Build binutils ===
cd "$BUILD_BINUTILS"
"$SRC_DIR/binutils-2.42/configure" --target=$TARGET --prefix=$PREFIX --with-sysroot --disable-nls --disable-werror
make -j$(nproc)
make install

# === Build gcc ===
cd "$BUILD_GCC"
"$SRC_DIR/gcc-13.2.0/configure" --target=$TARGET --prefix=$PREFIX --disable-nls --enable-languages=c --without-headers
